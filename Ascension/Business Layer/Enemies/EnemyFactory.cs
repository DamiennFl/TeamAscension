...
namespace Ascension
{
    ...
    internal abstract class EnemyFactory(ContentManager contentManager, GraphicsDevice graphicsDevice)
    {
        ...
        public abstract Enemy CreateEnemyA(Vector2 position, Vector2 velocity, int health, string bulletType, float shotsPerSecond);

        public abstract Enemy CreateEnemyB(Vector2 position, Vector2 velocity, int health, string bulletType, float shotsPerSecond);

        public abstract Enemy CreateMidBoss(Vector2 position, Vector2 velocity, int health, string bulletType, float shotsPerSecond);

        public abstract Enemy CreateFinalBoss(Vector2 position, Vector2 velocity, int health, string bulletType, float shotsPerSecond);
    }
}
